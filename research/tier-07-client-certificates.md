...
It does not take ownership. `mbedtls_pk_free()` in `extras/pk.c` line 56 carries the comment "The ownership of the priv_id key for opaque keys is external of the PK module" and guards the destroy with `pk_info->type != MBEDTLS_PK_OPAQUE`. A wrapped persistent key survives the context being freed. This is the single most dangerous thing that could have gone wrong and it does not.
