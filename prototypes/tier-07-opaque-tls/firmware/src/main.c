...
int main(void)
{
	...
	/*
	 * PROTOTYPE, issue #157. This image ends here and never reaches the OTA
	 * poll loop below.
	 *
	 * That is deliberate and it was learned the hard way. The first spike
	 * image ran, then polled the service, found a release waiting, installed
	 * it and rebooted into it, which replaced the spike with Tier 5 before
	 * anyone had read the console. A spike that updates itself away is a
	 * spike that cannot be rerun.
	 *
	 * So the spike repeats instead, and the watchdog is fed between runs by
	 * the same thread it vouches for.
	 */
	while (true) {
		health_gate_feed();
		course_opaque_tls_spike();
		printk("spike.sleep next attempt in 30 seconds\n");
		for (int i = 0; i < 6; i++) {
			health_gate_feed();
			k_sleep(K_SECONDS(5));
		}
