...
/* The image digest, computed by reading the secondary slot back.
 *
 * Tier 4 hashed each fragment as it was written, which cost one pass over
 * bytes that had to be copied anyway. Tier 5 cannot: a download that resumes
 * across a reboot has no hash state left after the power cut, and persisting
 * SHA-256 midstate would be a great deal of unpleasantness for no gain.
 *
 * Reading the slot back is the better check in any case. Tier 4's own comment
 * says check 6 protects the upgrade request rather than the flash write, and
 * hashing the slot makes that true of what is actually in the flash instead of
 * what the device believes it wrote. It also catches the one range-response
 * failure nothing earlier looks at, a response that began at the wrong offset.
 *
 * The read is a megabyte or so of flash and takes well under a second.
 */
int release_digest_slot(const struct flash_area *fa, size_t image_offset, size_t len,
			char *hex, size_t hex_len)
{
	...
	while (done < len) {
		...
		err = flash_area_read(fa, image_offset + done, buf, chunk);
		if (err != 0) {
			(void)psa_hash_abort(&op);
			printk("release.digest could not read the slot at %zu err=%d\n",
			       done, err);
			return err;
		}
		...
		if (status != PSA_SUCCESS) {
			(void)psa_hash_abort(&op);
			printk("release.digest could not hash the slot status=%d\n",
			       (int)status);
			return -EIO;
		}
		done += chunk;
	...
	printk("release.digest hashed %zu bytes read back out of the secondary slot\n", len);
	return 0;
}
...
bool release_policy_has_key(void)
{
	return RELEASE_PUBKEY_LEN > 0;
}
