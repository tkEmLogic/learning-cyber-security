...
static int cmd_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (course_identity_is_provisioned()) {
		...
		shell_print(sh, "provision.status factory fingerprint=sha256:%s",
			    course_identity_factory_fingerprint());
		if (course_identity_has_operational()) {
			shell_print(sh, "provision.status operational fingerprint=sha256:%s owner=%s",
				    course_identity_operational_fingerprint(),
				    course_identity_operational_owner() == NULL
					    ? "unreadable" : course_identity_operational_owner());
		} else {
			shell_print(sh, "provision.status no Operational certificate held, "
					"this device will not download");
		}
	} else {
...
/*
 * Reprint the nonce, the remaining time and the attempt state.
 *
 * #135 settled that this is allowed: the console already printed the nonce on
 * the press, so withholding it from a shell on the same wire buys nothing and
 * costs the Learner a walk back to the button. The control is the ten-minute
 * window, not the nonce staying secret over time.
 *
 * It does not open a window. Only the physical action does that.
 */
static int cmd_claim_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* printk rather than shell_print, so the lines are identical whether
	 * they came from the press or from this command, and the station's
	 * console filters see one format.
	 */
	course_claim_print_status();
	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(claim_commands,
	SHELL_CMD_ARG(status, NULL, "Reprint the claim nonce and the time remaining",
		      cmd_claim_status, 1, 0),
	SHELL_SUBCMD_SET_END
);

SHELL_CMD_REGISTER(claim, &claim_commands,
		   "The Claim window, opened by holding BOOT", NULL);
