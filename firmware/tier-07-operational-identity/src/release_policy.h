 ...
 * digest is computed by reading the slot back once the transfer is done.
 *
...
/* True when a Release manifest verification key is compiled into this image.
 * An image without one verifies no manifest, which the health gate treats as
 * unhealthy for the same reason it treats a missing trust anchor that way.
 */
