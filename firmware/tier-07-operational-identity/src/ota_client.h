/*
 * The Tier 7 OTA client.
 *
 * Tier 7 changes the transport under this interface and nothing above it. Every
 * exchange now runs over the opaque-key socket in opaque_tls.h, presenting a
 * client certificate whose private half is a PSA key identifier, so the service
 * can no longer be *told* who is calling. The request building and response
 * handling below are Tier 4's, unchanged, which is what #158 went and proved
 * was possible.
 *
 * The header that follows is Tier 4's and still describes what this client
 * does with what it fetches.
 *
 * ---
 *
...
/*
 * The device half of the claim, and every retrieval poll after it.
 *
 * One POST to /v1/devices/{device_id}/claim, presented with the FACTORY
 * identity, re-sent unchanged on every poll. The status and the body are both
 * returned: on this route every authorization refusal is 403, so the status is
 * never the reason, and the reason is the body's check field.
 *
 * Returns 0 when an exchange completed, whatever the service said. A negative
 * return is a transport failure and nothing was learned.
 */
