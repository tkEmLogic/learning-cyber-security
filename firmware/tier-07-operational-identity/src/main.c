...
{
	printk("ESP32-C6 Reference product: Tier 7, owner-scoped operational identity\n");
	...
	printk("Board: %s\n", CONFIG_BOARD_TARGET);
	announce_hardware();
	printk("Tier 7 boot mode: signed MCUboot images, swap using offset, TEST upgrade\n");
	printk("Tier 7 downgrade prevention: by security counter, enforced by the bootloader\n");
	...
	printk("Tier 7 keeps every Tier 6 control and changes who the device is allowed to be.\n");
	printk("It presents a client certificate on every connection to a restricted endpoint,\n");
	printk("with the private half staying inside PSA, so the service can no longer be told\n");
	printk("who is calling. The certificate that authorizes downloads is owner-scoped and\n");
	printk("is obtained by a two-party claim: a physical action here, and an Owner\n");
	printk("credential somewhere this device never talks to.\n");
	printk("This device cannot evaluate a certificate validity window. It has no clock to\n");
	printk("do it with, CONFIG_MBEDTLS_HAVE_TIME_DATE is off, and it will present an\n");
	printk("expired Operational certificate until the service refuses it. That is a\n");
	printk("published limit of this tier rather than an oversight: the service enforces\n");
	printk("expiry, and device time stays evidence rather than an authorization input.\n");
	...
	printk("Restricted endpoints are reached over the course opaque-key socket, not\n");
	printk("Zephyr's TLS socket layer. Zephyr's credential store cannot carry a PSA key\n");
	printk("identifier as a client private key, so this image registers its own socket\n");
	printk("implementation and reaches the key through mbedtls_pk_wrap_psa(). No Zephyr\n");
	printk("source is changed and no module is forked.\n");
...
int main(void)
{
	...
	while (true) {
		 ...
		 *
		 * #158 made one exchange at a time a decision rather than a
		 * limitation: one socket, one http_client_req(), on main. So
		 * the claim cannot run beside the OTA poll, and while a window
		 * is open the claim is what main is for. A window lasts ten
		 * minutes at most and an unclaimed device has nothing to
		 * download anyway.
		 */
#ifdef CONFIG_COURSE_CLAIM
		if (course_claim_window_open()) {
			course_claim_step();
			continue;
		}
#endif
...
static void reopen_work_handler(struct k_work *work)
{
	...
	printk("provision.gate the button was held for %d seconds.\n",
	       CONFIG_COURSE_PROVISIONING_BUTTON_HOLD_SECONDS);
	printk("provision.gate From Tier 7 this one act does two things: it opens the\n");
	printk("provision.gate provisioning interface, which is remanufacturing, and it opens\n");
	printk("provision.gate a Claim window, which is ownership. One physical act therefore\n");
	printk("provision.gate reaches both, and physical access is the entire boundary. That\n");
	printk("provision.gate is a trade Tier 6 already made and Tier 7 inherits.\n");
	course_provisioning_open();
#ifdef CONFIG_COURSE_CLAIM
	/*
	 * The press generates, as section 8 words it. Nothing is typed to cause
	 * it and the nonce prints immediately.
	 *
	 * This runs on the system workqueue, which is why CONFIG_SYSTEM_WORKQUEUE_STACK_SIZE
	 * is raised in prj.conf: generating a P-256 key and writing a
	 * certification request needs more than the default 1024 bytes, and
	 * #138 warned about exactly this class of overrun reading as a fault
	 * inside the crypto library. The key is volatile, so no Secure Storage
	 * record is decrypted here; the request writer is the expensive part.
	 */
	course_claim_open_window();
#endif
}
...
void course_identity_gate(void)
{
	/* The provisioning shell is not built. */
}
