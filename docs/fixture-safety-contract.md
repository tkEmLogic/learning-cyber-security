...
| Reset appends a `fixture_reset` entry, deletes nothing from `records.jsonl` or `events.jsonl`, and clears only the adversary owner entry and the serials the fixture marked | `bypassReset`, `removeOwnerEntries` and `removeRevokedSerials` in `internal/courseapp/tier07_bypass.go` | Enforced |
