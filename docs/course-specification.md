...
**Implementation requirement.** Use MCUboot swap-using-offset, not swap-with-scratch, overwrite-only, or direct-XIP mode. Enable `BOOT_SWAP_USING_OFFSET`, `BOOT_VALIDATE_SLOT0`, and one updateable image. Swap-using-offset needs no scratch area, so the scratch partition in the flash map above stays defined and unused rather than the map moving. Select the mode explicitly rather than leaving it to the default that follows from an absent scratch node. The mode requires every sector in both slots to be the same size, which the ESP32-C6 satisfies, and it places a downloaded image one sector into the secondary slot rather than at its start; Zephyr's image utilities do that themselves once the application knows the bootloader mode. Starting in Tier 3, use ECDSA P-256 image signatures. Starting in Tier 4, enable `MCUBOOT_DOWNGRADE_PREVENTION` and `MCUBOOT_DOWNGRADE_PREVENTION_SECURITY_COUNTER`. Put the same security-counter value in the signed MCUboot image TLV and the signed Release manifest. The human-readable firmware version stays separate. A candidate counter may equal the confirmed image counter for an ordinary feature release, but it may never be lower. This is software downgrade prevention because a physical attacker can replace boot state.

**Implementation requirement.** The application writes only to the secondary slot. From Tier 5 it requests `BOOT_UPGRADE_TEST` and never a permanent upgrade. Tier 3 and Tier 4 request a permanent upgrade on purpose, because test boot, the health gate, confirmation, and revert are Tier 5's subject; the fallback path physically exists from Tier 0 because of the swap mode, and no earlier tier uses it. MCUboot owns the swap, trial boot, and revert state. The application calls `boot_write_img_confirmed()` only after the local health gate passes. There is no separate application-managed accepted-counter database, so a failed trial can revert to the previously confirmed image without a newer counter making that image ineligible. Power-cut tests cover download, trailer update, every swap phase, first boot, health checking, confirmation, and the first reboot after confirmation.
...
**Implementation requirement.** The OTA service runs locally in a container on the Learner's computer. The container engine is Podman on every platform, decided in issue [#156](https://github.com/tkEmLogic/learning-cyber-security/issues/156): Docker Desktop is not free for company use above a small size threshold and this course is taken at work, and Podman's rootless model lines USB device access up with ordinary user permissions. The first release supports current Linux distributions, and macOS and Windows through Podman Desktop, where a board cannot be flashed. The dev container image is published to the GitHub Container Registry so a Learner pulls it rather than building it. Ubuntu 24.04 is the CI and reference environment, not the only supported host. The Learner's computer and the ESP32-C6 join the same Wi-Fi network. Setup includes a course-local certificate authority, server certificate, sample releases, and deterministic test data. The service binds only to the configured classroom interface by default. After dependencies and container images are installed, the core update exercises do not depend on an external cloud service. Test cases include a corrupted image, modified release manifest, expired or untrusted server certificate, interrupted download, replayed release, failed health check, and successful revert.
...
**Fixed decision.** Each hardening-tier module uses an incident-driven opening, a linear work procedure, and an evidence-first close, in this exact order: tier title; scenario; learning result; safety boundary; starting state; weakness ledger before the work; controlled attack or failure reproduction; investigation questions and trust-boundary explanation; ordered work procedure with commands and expected results; replay or re-analysis of the original observation; bypass and failure tests where applicable; weakness ledger after the work; security claim and evidence status; what this tier found in the earlier tiers; security evidence pack update; troubleshooting; informal Mentor conversation; transition and attack preview for the next tier; primary references.

**Amendment.** The order gained one section after Tier 7 was published. Every control tier so far found a defect in an earlier tier, and Tiers 5, 6 and 7 each invented the same heading in the same place to report it before anything asked them to. It is now part of the order, between security claim and evidence status and the security evidence pack update. A tier that found nothing says so in one line rather than leaving the section out. Source: resolved ticket [#145](https://github.com/tkEmLogic/learning-cyber-security/issues/145).

**Amendment.** The order binds in two strengths, which `docs/agents/course-module-template.md` states in full. The opening, the ledgers and the evidence-first close are a fixed spine whose headings are the same string in every module. The five sections from the attack reproduction through the bypass tests are the work band: each is named after what the tier actually does, a tier may omit one when it has no such step, and a tier that omits one says so in its own text where the step would have been. A module may also insert sections of its own between the listed ones, which is how this order gained the section above. Source: resolved ticket [#145](https://github.com/tkEmLogic/learning-cyber-security/issues/145).
...
**Clarification.** There are three module variants and there is no lifecycle variant. The `kind` each tier carries in `course.yml` says where the tier sits in the course arc, and from Tier 6 that value is `lifecycle`; it selects no module shape, and no code reads it. Tiers 6 through 9 are control tiers by the sentence above, and what makes them lifecycle tiers is what the control is rather than how the module is written. Source: resolved ticket [#145](https://github.com/tkEmLogic/learning-cyber-security/issues/145).
...
| Boot state machine | The fixed flash map and swap-using-offset configuration in section 6 pass power-cut, test-boot, confirmation, revert, downgrade, and signed serial-recovery tests without accepting an unsigned image or losing the documented recovery path. |
